// Hook para envio de imagens via Z-API /send-image e gateway WhatsApp
routerAdd('POST', '/backend/v1/whatsapp/enviar-imagem', (e) => {
  try {
    ...
    const imageData = (body.imagem || body.image || body.base64 || body.imagem_url || '').trim()
    const legenda = (body.legenda || body.caption || body.mensagem || '').trim()
    const nomeArquivo = (body.nome_arquivo || body.fileName || 'imagem.jpg').trim()
    ...
    msgRecord.set('tipo_mensagem', 'imagem')
    ...
    // Se temos base64 com prefixo ou URL externa, salvar documento_url se aplicável
    if (imageData && (imageData.startsWith('http') || imageData.startsWith('data:image'))) {
      msgRecord.set('documento_url', imageData)
    }
    ...
    if (!rawApiUrl) {
      ...
      if (finalClienteId) {
        try {
          ...
          atvRec.set('titulo', 'Imagem por WhatsApp (Falha de Envio)')
          atvRec.set(
            'descricao',
      ...
      return e.json(200, {
        ...
        status: 'falha',
        message:
          'Imagem registrada no histórico como "falha", pois os Secrets WHATSAPP_API_URL e WHATSAPP_API_KEY não foram preenchidos.',
        data: msgRecord,
      })
    }
    ...
    // Determinar o payload da imagem (URL pública ou Base64)
    let payloadImageValue = imageData
    if (!payloadImageValue && msgRecord.getString('arquivo')) {
    ...
    if (isZApi) {
      ...
      if (zapiMatch) {
        ...
        targetUrl = hostPrefix + '/instances/' + instanceId + '/token/' + token + '/send-image'
      } else {
        targetUrl = baseWithoutSuffix + '/send-image'
      }
      ...
      targetUrl = cleanUrl.replace(/\/+$/, '') + '/send-image'
      if (apiKey) {
    ...
    console.log(
      '[WHATSAPP IMAGE SEND INICIADO]',
      JSON.stringify({
        targetUrlMasked: maskedTarget,
        cleanPhone: cleanPhone,
        fileName: nomeArquivo,
        hasImage: Boolean(payloadImageValue),
      }),
    )
    ...
    try {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        if (conversaId) {
          try {
            ...
            console.log('[CONV UPDATE IMAGE AVISO]', errConv)
          }
        }
        ...
        if (finalClienteId) {
          try {
            ...
            atvRec.set('cliente_id', finalClienteId)
            atvRec.set('tipo', 'follow_up')
            atvRec.set('titulo', 'Imagem enviada por WhatsApp')
            atvRec.set(
              'descricao',
            ...
            console.log('[ATV ERRO IMAGE SUCESSO]', errAtv)
          }
        }
        ...
        return e.json(200, {
          ...
          sent: true,
          status: 'enviada',
          message: 'Imagem enviada com sucesso via WhatsApp!',
          data: msgRecord,
        })
      } else {
        ...
        console.log('[WHATSAPP IMAGE FALHA HTTP]', logMsg)
      ...
      console.log('[WHATSAPP IMAGE EXCECAO HTTP]', errMsg)
    ...
    let msg = 'Erro interno ao enviar imagem'
    if (err && err.message) msg = err.message
    console.log('[WHATSAPP IMAGE ERRO INTERNO]', msg)
